...
#pragma mark - 初始化
- (void)setUpUIView{
    self.pageChangeMode = PageChangeMode_Default;
    self.rollDirection = RollDirection_Horizontal;
    self.pagePosition = PagePosition_None;
    [self addSubview:self.scrollView];
    [self addSubview:self.pageControl];
...
- (void)nextPage{
    if (self.pageChangeMode == PageChangeMode_Fade) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (self.rollDirection == RollDirection_Horizontal) {
        ...
        if (offset.x < self.scrollViewWidth * 2) {   //右滑
            if (self.pageChangeMode == PageChangeMode_Fade) {
                self.currentView.alpha = offset.x / self.scrollViewWidth - 1;
                self.nextView.alpha = 2 - offset.x / self.scrollViewWidth;
            } else{
            ...
            if (self.pageChangeMode == PageChangeMode_Fade) {
                self.nextView.alpha = offset.x / self.scrollViewWidth - 2;
                self.currentView.alpha = 3 - offset.x / self.scrollViewWidth;
            } else{
        ...
        if (offset.y < self.scrollViewHeight * 2) { //向下滑
            if (self.pageChangeMode == PageChangeMode_Fade) {
                self.currentView.alpha = offset.y / self.scrollViewHeight - 1;
                self.nextView.alpha = 2 - offset.y / self.scrollViewHeight;
            } else{
            ...
            if (self.pageChangeMode == PageChangeMode_Fade) {
                self.nextView.alpha = offset.y/self.scrollViewHeight - 2;
                self.currentView.alpha = offset.y/self.scrollViewHeight - 3;
            } else{
...
- (void)setUpSubViewOfScrollView{
    if (self.dataArray.count > 1) {
        ...
        if (self.pageChangeMode == PageChangeMode_Fade) {
            ...
            [self insertSubview:self.nextView atIndex:1];
        }else{
