...
// 分页控件位置
typedef enum {
    PagePosition_None,          // 默认值 == PagePosition_BottomCenter
...
// 滚动方向
typedef enum {
    RollDirection_Horizontal,  // 水平方向 默认值
    RollDirection_Vertical     // 竖直方向
}RollDirection;

// 分页控件切换方式
typedef enum {
    PageChangeMode_Default,   // 轮播滚动
    PageChangeMode_Fade       // 淡入淡出
}PageChangeMode;
 ...
 * 分页控件切换方式
 */
@property (nonatomic, assign) PageChangeMode pageChangeMode;
